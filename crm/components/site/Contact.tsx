...
export default function Contact() {
  return (
    <section id="contact" style={{ padding: '120px 24px', background: 'linear-gradient(180deg, #0D0905 0%, #110A06 50%, #0D0905 100%)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, alignItems: 'start' }} className="contact-grid">
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                {item.href ? (
                  <a href={item.href}
                    ...
                    onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = 'rgba(244,168,92,0.35)'; el.style.transform = 'translateY(-4px)'; el.style.boxShadow = '0 16px 48px rgba(0,0,0,0.3), 0 0 24px rgba(244,168,92,0.08)'; el.style.background = 'rgba(244,168,92,0.05)'; }}
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
            style={{ borderRadius: 24, overflow: 'hidden', background: 'linear-gradient(145deg, rgba(244,168,92,0.07), rgba(156,74,30,0.07))', minHeight: 380, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, padding: 44, border: '1px solid rgba(255,255,255,0.07)' }}>
            <div style={{ width: 84, height: 84, borderRadius: 24, background: 'linear-gradient(135deg, #E08A3C, #F4A85C)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 38, boxShadow: '0 8px 32px rgba(244,168,92,0.3)' }}>📍</div>
            <div style={{ textAlign: 'center' }}>
              ...
              <a href="https://maps.google.com" target="_blank" rel="noopener noreferrer"
                style={{ display: 'inline-block', padding: '13px 30px', borderRadius: 99, fontWeight: 700, fontSize: 14, background: 'linear-gradient(135deg, #E08A3C, #F4A85C)', color: '#0d0a00', textDecoration: 'none', boxShadow: '0 4px 20px rgba(244,168,92,0.3)', transition: 'all 0.22s' }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.boxShadow = '0 8px 32px rgba(244,168,92,0.5)'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-2px)'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow = '0 4px 20px rgba(244,168,92,0.3)'; (e.currentTarget as HTMLElement).style.transform = 'translateY(0)'; }}>
                Deschide în Maps →
              </a>
            </div>
          </motion.div>
