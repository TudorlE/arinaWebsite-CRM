...
export default function Founder() {
  return (
    <section id="fondator" style={{ padding: '120px 24px', background: 'linear-gradient(180deg, #0a0618 0%, #0e0825 50%, #0a0618 100%)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 88, alignItems: 'center' }} className="founder-grid">
        ...
        <motion.div initial={{ opacity: 0, x: -60 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }}>
          <span style={{ display: 'inline-block', padding: '7px 18px', borderRadius: 99, fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 24, background: 'rgba(167,139,250,0.1)', color: '#A78BFA', border: '1px solid rgba(167,139,250,0.2)' }}>
            Fondatoarea Școlii
          </span>
          ...
          <h2 style={{ fontSize: 'clamp(42px, 5vw, 72px)', fontWeight: 900, color: '#FFD54A', lineHeight: 1.0, letterSpacing: '-0.03em', margin: '0 0 36px', textShadow: '0 0 40px rgba(255,213,74,0.25)' }}>Bădulescu</h2>
          ...
          <motion.blockquote initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }}
            style={{ borderLeft: '3px solid #FFD54A', paddingLeft: 24, margin: 0, borderRadius: '0 0 0 4px' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,213,74,0.7)', margin: 0 }}>Fondatoare & Director Artistic, Arry Production</p>
            </div>
          </motion.blockquote>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: 60 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }}
          ...
          {/* Ambient glow */}
          <div style={{ position: 'absolute', inset: -30, background: 'radial-gradient(ellipse, rgba(167,139,250,0.15) 0%, rgba(255,213,74,0.08) 50%, transparent 70%)', borderRadius: 40, pointerEvents: 'none' }} />
          <div style={{ position: 'relative', zIndex: 1, borderRadius: 28, overflow: 'hidden', boxShadow: '0 32px 80px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(10,6,24,0.65) 0%, transparent 50%)' }} />
          </div>
          <span className="fa" style={{ position: 'absolute', top: -36, right: -28, fontSize: 80, opacity: 0.1, zIndex: 2, userSelect: 'none', color: '#FFD54A', filter: 'drop-shadow(0 0 16px rgba(255,213,74,0.4))' }}>♫</span>
          <span className="fb" style={{ position: 'absolute', bottom: -20, left: -24, fontSize: 60, opacity: 0.09, zIndex: 2, userSelect: 'none', color: '#A78BFA' }}>♩</span>
        </motion.div>
      </div>
