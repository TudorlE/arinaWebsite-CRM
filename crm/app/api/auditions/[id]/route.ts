...
export async function GET(request: NextRequest, { params }: Params) {
  ...
  const { data, error } = await supabase.from('auditions').select('*, students(name), teachers(name)').eq('id', id).single();
  ...
  const { students, teachers, ...audition } = data as {
    students: { name: string } | null; teachers: { name: string } | null; [key: string]: unknown;
  };
  return NextResponse.json({ audition: { ...audition, student_name: students?.name ?? null, teacher_name: teachers?.name ?? null } });
}
...
export async function PUT(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const allowed = ['student_id', 'teacher_id', 'discipline', 'date', 'time', 'duration', 'notes', 'result', 'status'];
