...
function Access({ roles }: { roles: ('admin' | 'teacher' | 'student')[] }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-5">
      ...
      {roles.map(r => <Badge key={r} variant={map[r].variant}>{map[r].label}</Badge>)}
    </div>
  );
}
...
function Callout({ type, label, children }: { type: 'tip' | 'warn'; label: string; children: React.ReactNode }) {
  ...
  return (
    <div className={`relative flex gap-3.5 rounded-2xl border p-4 pl-5 my-6 text-sm overflow-hidden
      ${isTip ? 'bg-accent-50/70 dark:bg-accent-900/15 border-accent-200/70 dark:border-accent-800/60' : 'bg-amber-50/70 dark:bg-amber-900/15 border-amber-200/70 dark:border-amber-800/60'}`}>
      <span className={`absolute left-0 top-0 bottom-0 w-1 ${isTip ? 'bg-accent-400' : 'bg-amber-400'}`} />
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center
        ${isTip ? 'bg-accent-100 dark:bg-accent-900/40 text-accent-600 dark:text-accent-400' : 'bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400'}`}>
        <Icon className="w-4 h-4" />
      </div>
      <div className="min-w-0">
        <p className={`font-bold mb-0.5 ${isTip ? 'text-accent-700 dark:text-accent-400' : 'text-amber-700 dark:text-amber-400'}`}>{label}</p>
...
function Steps({ items }: { items: React.ReactNode[] }) {
  return (
    <ol className="relative space-y-4 my-5 ml-1">
      <span className="absolute left-[13px] top-2 bottom-2 w-px bg-slate-200 dark:bg-slate-800" aria-hidden />
      {items.map((item, i) => (
...
function FieldTable({ rows }: { rows: [string, string][] }) {
  return (
    <div className="my-5 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="text-left text-[11px] font-bold uppercase tracking-wider text-slate-400 bg-slate-50 dark:bg-slate-800/60">
        ...
        <tbody>
          {rows.map(([field, desc], i) => (
            <tr key={field} className={i % 2 === 1 ? 'bg-slate-50/60 dark:bg-slate-800/20' : ''}>
              <td className="py-2.5 pl-4 pr-4 font-semibold text-slate-800 dark:text-slate-200 whitespace-nowrap align-top">{field}</td>
              <td className="py-2.5 pr-4 text-slate-500 dark:text-slate-400 align-top">{desc}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function Section({ meta, sub, children }: { meta: SectionMeta; sub: string; children: React.ReactNode }) {
  ...
  return (
    <section id={meta.id} className="scroll-mt-8 py-11 border-t border-slate-100 dark:border-slate-800/80 first:border-t-0 first:pt-0" data-guide-section>
      <div className="flex items-center gap-3 mb-1.5">
        ...
        <h2 className="text-xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">{meta.title}</h2>
      </div>
...
export default function GhidPage() {
  ...
  return (
    <div className="flex flex-col flex-1">
      <div className="relative overflow-hidden bg-gradient-to-r from-brand-700 via-brand-600 to-accent-600 px-8 py-7 shadow-lg">
        ...
        <div className="relative flex items-center gap-4">
          <div className="p-3 bg-white/15 rounded-2xl backdrop-blur-sm ring-1 ring-white/20"><BookOpen className="w-7 h-7 text-white" /></div>
          <div>
            <h1 className="text-2xl font-extrabold text-white tracking-tight">Ghid de utilizare</h1>
            <p className="text-white/75 text-sm font-medium mt-0.5">Cum se folosește CRM-ul Arry Production — pas cu pas</p>
          </div>
        </div>
      </div>

      {/* Reading progress */}
      <div className="h-[3px] bg-slate-100 dark:bg-slate-800 flex-shrink-0">
      ...
      <main ref={mainRef} onScroll={handleScroll} className="flex-1 overflow-y-auto relative">
        <div className="max-w-6xl mx-auto flex gap-10 px-6 py-8">
          ...
          <aside className="hidden lg:block w-60 flex-shrink-0 sticky top-8 self-start max-h-[calc(100vh-6rem)] overflow-y-auto">
            <div className="relative mb-4">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-shadow"
              />
            </div>
            <nav className="space-y-5">
              {GROUPS.map(group => {
                ...
                return (
                  <div key={group}>
                    <p className="text-[10px] font-bold uppercase tracking-wider text-slate-300 dark:text-slate-600 mb-1.5 px-2.5">{group}</p>
                        ...
                        return (
                          <a
                            ...
                            className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[13px] font-medium transition-colors
                              ${isActive ? 'bg-brand-50 dark:bg-brand-900/30 text-brand-700 dark:text-brand-300' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/60 hover:text-slate-700 dark:hover:text-slate-200'}`}
                          >
                            <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${isActive ? 'text-brand-600 dark:text-brand-400' : 'text-slate-400'}`} />
          ...
          <div className="min-w-0 flex-1 max-w-3xl">
            ...
            <Section meta={SECTIONS[0]} sub="Ce este acest CRM și cum e organizat">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-4 leading-relaxed">
                Te autentifici cu adresa de email și parola contului tău. Dacă ești profesor sau elev și tocmai te-ai înregistrat, contul tău așteaptă <b>aprobarea</b> unui administrator înainte de a putea intra (vezi <a href="#aprobari" className="text-brand-600 dark:text-brand-400 font-medium">Aprobări</a>).
              </p>
            ...
            <Section meta={SECTIONS[1]} sub="Cine ce poate face în CRM">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 hover:border-brand-300 dark:hover:border-brand-700 transition-colors">
                  <Badge variant="indigo">Admin</Badge>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Acces complet: adaugă/editează/șterge orice, aprobă conturi noi, schimbă roluri, vede toate rapoartele.</p>
                </div>
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 hover:border-accent-300 dark:hover:border-accent-700 transition-colors">
                  <Badge variant="purple">Profesor</Badge>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Vede și editează propriul program și proprii elevi. Nu vede Aprobări, Roluri, Cereri Site sau Orar Fix.</p>
                </div>
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
                  <Badge variant="gray">Elev</Badge>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Acces doar de vizualizare la propriul program.</p>
                </div>
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
                  <p className="font-bold text-sm text-slate-800 dark:text-slate-200">Legătura Profesor ↔ cont</p>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Un cont de profesor trebuie <b>asociat</b> cu o fișă de profesor ca să funcționeze filtrarea „doar datele mele”.</p>
                </div>
              </div>
            </Section>
            ...
            <Section meta={SECTIONS[3]} sub="Lista completă a elevilor + fișa fiecăruia">
              ...
              <h3 className="font-bold text-slate-900 dark:text-slate-100 mt-6 mb-2">Adaugă un elev nou</h3>
              <Steps items={[
              ...
              <h3 className="font-bold text-slate-900 dark:text-slate-100 mt-6 mb-2">Butoanele de pe fiecare card</h3>
              <ul className="list-disc list-outside pl-5 space-y-1.5 text-sm text-slate-600 dark:text-slate-400 mb-2">
            ...
            <Section meta={SECTIONS[9]} sub="Calendarul complet, cu filtre și toate opțiunile de editare">
              ...
              <h3 className="font-bold text-slate-900 dark:text-slate-100 mt-5 mb-2">Adaugă o lecție</h3>
              <Steps items={[
              ...
              <h3 className="font-bold text-slate-900 dark:text-slate-100 mt-5 mb-2">Editează sau mută o lecție</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-2 leading-relaxed">
            ...
            <Section meta={SECTIONS[14]} sub="Mesajele primite din formularul de contact al site-ului public">
              <Access roles={['admin']} />
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                Fiecare persoană care completează formularul de pe site apare aici ca o cerere nouă. Mută-o prin etapele fluxului — <code className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 font-mono text-xs">nou → contactat → înscris → anulat</code> — pe măsură ce discuți cu ea.
              </p>
            </Section>

            <Section meta={SECTIONS[15]} sub="Gestionarea conturilor de utilizator">
              <Access roles={['admin']} />
              <h3 className="font-bold text-slate-900 dark:text-slate-100 mb-2">Aprobări</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-2 leading-relaxed">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-4 leading-relaxed">Poți și <b>respinge</b> o cerere — contul rămâne blocat până e reaprobat.</p>
              <h3 className="font-bold text-slate-900 dark:text-slate-100 mb-2">Roluri</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
            ...
            <Section meta={SECTIONS[17]} sub="">
              <div className="space-y-5">
                <div>
                  <p className="font-bold text-sm text-slate-900 dark:text-slate-100 mb-1">Care e diferența dintre Program și Program General?</p>
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed"><b>Program</b> e varianta clasică, simplă (săptămânal/lunar, drag &amp; drop). <b>Program General</b> e varianta completă, cu filtre și toate opțiunile de propagare pentru lecțiile recurente.</p>
                </div>
                <div>
                  <p className="font-bold text-sm text-slate-900 dark:text-slate-100 mb-1">Am anulat din greșeală o lecție — ce fac?</p>
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">Deschide lecția din Program General, apasă Editează și schimbă statusul înapoi la „Programată”, sau la „Recuperată” dacă elevul a recuperat-o ulterior.</p>
                </div>
                <div>
                  <p className="font-bold text-sm text-slate-900 dark:text-slate-100 mb-1">De ce nu apare un profesor nou în filtrele „doar datele mele”?</p>
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">Foarte probabil contul lui de utilizator nu a fost asociat cu fișa de profesor la aprobare. Mergi în Aprobări pentru a face legătura.</p>
                </div>
                <div>
                  <p className="font-bold text-sm text-slate-900 dark:text-slate-100 mb-1">Cum recuperez o lecție anulată la o altă dată?</p>
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">Adaugă o lecție nouă în Program General la data de recuperare și marchează-i statusul ca <b>„Recuperată”</b> — apare corect în statisticile de la Elevi Frecvență și Profesori Frecvență.</p>
                </div>
                <div>
                  <p className="font-bold text-sm text-slate-900 dark:text-slate-100 mb-1">Unde văd câți bani a adus un elev sau un profesor de când e cu noi?</p>
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">Apasă butonul <b>Detalii</b> de pe cardul lui, din Elevi General sau Profesori General.</p>
                </div>
              </div>
            </Section>
