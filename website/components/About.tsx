...
export default function About() {
  return (
    <section id="despre" style={{ padding: '120px 24px', background: 'linear-gradient(180deg, #07040f 0%, #0c0720 50%, #07040f 100%)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }} className="about-grid">
        ...
        <motion.div initial={{ opacity: 0, x: -60 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }}
          ...
          {/* Glow behind */}
          <div style={{ position: 'absolute', inset: -24, background: 'radial-gradient(ellipse, rgba(255,213,74,0.12) 0%, transparent 70%)', borderRadius: 40, pointerEvents: 'none' }} />
          <div style={{ borderRadius: 28, overflow: 'hidden', boxShadow: '0 32px 80px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.05)', position: 'relative' }}>
            ...
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(7,4,15,0.4) 0%, transparent 60%)' }} />
          </div>
          <motion.div initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }}
            style={{ position: 'absolute', bottom: -22, right: -22, background: 'rgba(8,5,20,0.92)', backdropFilter: 'blur(16px)', borderRadius: 18, padding: '18px 26px', boxShadow: '0 16px 48px rgba(0,0,0,0.4)', border: '1px solid rgba(255,213,74,0.2)' }}>
            <p style={{ fontSize: 38, fontWeight: 900, color: '#FFD54A', margin: 0, lineHeight: 1, textShadow: '0 0 24px rgba(255,213,74,0.4)' }}>10+</p>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.55 }}
            style={{ position: 'absolute', top: -22, left: -22, background: 'rgba(8,5,20,0.92)', backdropFilter: 'blur(16px)', borderRadius: 18, padding: '18px 26px', boxShadow: '0 16px 48px rgba(0,0,0,0.4)', border: '1px solid rgba(255,213,74,0.2)' }}>
            <p style={{ fontSize: 38, fontWeight: 900, color: '#FFD54A', margin: 0, lineHeight: 1, textShadow: '0 0 24px rgba(255,213,74,0.4)' }}>300+</p>
        ...
        {/* Text */}
        <motion.div initial={{ opacity: 0, x: 60 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }}>
          <span style={{ display: 'inline-block', padding: '7px 18px', borderRadius: 99, fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 22, background: 'rgba(255,213,74,0.1)', color: '#FFD54A', border: '1px solid rgba(255,213,74,0.2)' }}>
            Despre noi
          </span>
          <h2 style={{ fontSize: 'clamp(34px, 4vw, 52px)', fontWeight: 900, color: '#fff', lineHeight: 1.08, letterSpacing: '-0.025em', margin: '0 0 20px' }}>
            Despre <span style={{ color: '#FFD54A' }}>Arry Production</span>
          </h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {vals.map((v, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                ...
                onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.background = 'rgba(255,213,74,0.06)'; el.style.borderColor = 'rgba(255,213,74,0.15)'; }}
